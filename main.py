...
def process_pdf(input_pdf_bytes, page_callback):
	...
	for page_number in range(num_pages):
		...
		# todo: limit page width to not be too big (and maybe make configurable)
...
st.markdown('<div style="text-align: center;font-size:300%;margin-bottom: 30px"><b>PDF Dark Mode Converter 🌗</b></div>', unsafe_allow_html=True)
	...
	if st.button("🖨️ START 🖨️", use_container_width=True, key="start_button"):
		...
		for i, uploaded_file in enumerate(uploaded_files):
			...
			with st.spinner(f"Processing {uploaded_file.name}"):
				...
				output_pdf_stream = process_pdf(input_pdf_stream.getvalue(), page_callback)
